...
draws %>%
  ...
  ggplot(mapping = aes(x = trump_share, fill = state)) + 
  geom_density() +
  facet_wrap(~state)

draws
#higher degree of certainty in Georgia
```
...
#map 100 repitions of a simulation
elections <- tibble(simulations = map(rep(1,1000), simulate_election))

elections %>%
  #tilda, dot refers back to simulations data, $ filters for PA
...
elections %>%
  ...
  mutate(both = case_when(
    pa_win == TRUE & oh_win == TRUE ~ TRUE, TRUE ~ FALSE)) %>%
  summarise(pa_avg = mean(pa_win), oh_avg = mean(oh_win), both_avg = mean(both))
...
#Loses Ohio = .07, Wins Ohio = .312369
#
```
...
elections_100 %>%
  mutate(df_pivoted = map(simulations, pivot_longer(.,cols = 1, 
                                                    names_to = "state",
